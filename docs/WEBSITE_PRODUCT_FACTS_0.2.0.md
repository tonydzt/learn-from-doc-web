...
资料基于当前代码实现整理。本文档按 0.2.0 内容状态整理；
...
- 用户在受支持的文档页面打开扩展 popup 后，可以创建索引、重建已有索引，或在存在断点时恢复索引。
- 创建索引时，扩展从当前文档侧边栏收集页面链接，并在后台逐页打开页面以测量正文高度。
- popup 在索引过程中显示收集链接、测量页面和保存索引的进度状态，并展示当前测量页标题、页数进度和进度条。
- 索引过程中会保存临时断点；如果前一次索引未完成且侧栏链接仍匹配，popup 会显示恢复入口和已完成页数。
...
- URL 只发生 hash 锚点变化时，扩展会避免不必要的 tracker 重启，以减少注入 UI 重新渲染造成的闪烁。
- 阅读记录可受站点级开关、单页开关和全局“新页面默认记录”设置共同控制。

### 3. 在文档页面展示进度

- 已索引且启用阅读进度的文档页面会在侧边栏顶部显示总进度卡片和进度条。
- 侧边栏中的已索引页面链接会显示单页进度徽章。
- 对没有可阅读正文高度的目录占位页，侧边栏徽章会显示 `Subdirectory`，而不是显示 `0%`。
- 已产生阅读记录的普通页面徽章支持删除交互；hover 时会从百分比切换为删除图标，点击或键盘触发后删除该页进度。
- 页面右侧可以显示细长的阅读地图，标识已读区间和当前视口所在位置。
- 阅读地图可在设置中关闭。
- 页面右下角可以显示单页阅读记录开关，用于暂停或恢复当前页面的进度写入；该开关支持在当前标签页会话内拖动位置。

### 4. Popup 概览与操作

- popup 展示当前文档范围名称、索引状态、总进度和已索引页面数量。
- popup 提供创建索引、重建索引和恢复索引按钮。
- popup 在存在未完成索引断点时展示恢复提示，包括已测量页数和总页数。
- 对内置站点之外的 HTTPS 页面，popup 提供文档框架检测操作；检测到支持的框架后，可继续建立该站点索引。
- 框架检测结果会按 host 和文档范围缓存；后续打开同一范围页面时，可基于缓存恢复支持状态。
- 对需要持久 origin 权限的检测站点，创建索引操作会请求授权；授权成功后可由后台接续启动索引，减少用户回到 popup 再操作一次的步骤。
- popup 可打开扩展管理页，并在当前页面属于已索引站点时跳转到对应站点详情。
...
- 全局设置包含 `Show right-side reading map`、`Record new pages by default`、`Index timing debug logs` 和界面语言选择。
- `Record new pages by default` 关闭后，已索引页面默认不写入新阅读进度，用户可以通过页面内单页开关开始记录。

### 6. 导入与导出

- 管理页可导出全部站点或单个站点的便携数据文件。
- 用户可以选择导出内容是否包含阅读进度；默认界面状态不勾选 `Include reading progress`。
- 便携数据包含站点索引、页面记录和站点设置；导出全部站点时也可包含全局应用设置。
...
| Starlight | Netlify Docs (`https://docs.netlify.com/`)、Pydantic Docs (`https://docs.pydantic.dev/latest/`)、sharp (`https://sharp.pixelplumbing.com/`) |
| Material for MkDocs | FastAPI (`https://fastapi.tiangolo.com/`)、Typer (`https://typer.tiangolo.com/`) |
| Retype | SillyTavern Docs (`https://docs.sillytavern.app/`)、Camoufox (`https://camoufox.com/`)、TurboStack Docs (`https://docs.turbostack.app/`) |

这些人工测试站点来自 adapter 源码注释，用于验证框架适配目标；它们不等同于完整、永久保证的网站兼容名单。代码注释明确提到 Cloudflare Developer Docs 当前不作为 Starlight 支持目标处理，官网不应将其列为已支持站点。

- Fumadocs adapter 对 `www.fumadocs.dev/docs/ui` 启用从 Next flight 数据补充页面链接的站点级覆盖配置。
- Netlify Docs 启用索引测量前等待页面加载完成的站点级覆盖配置，以避免图片较多时低估正文高度。
- Typer 启用首次正文节点稳定等待的站点级覆盖配置，以适配 Material for MkDocs instant navigation。
- Retype adapter 默认启用索引测量前等待页面加载完成。
...
说明：语言列表来自设置类型定义。官网可以表述为“提供 12 种可选界面语言”。当前代码中英文文案最完整，其他语言通过部分翻译叠加英文默认文案；官网不应仅凭语言列表断言每种语言的全部文案翻译覆盖度相同。

## 数据存储与权限事实

### 本地数据

- 文档站点索引、页面记录、阅读进度、站点设置、单页设置和索引断点保存在扩展的本地 IndexedDB 数据库中。
- IndexedDB 当前数据库名为 `developer-docs-progress-tracker`，当前 schema 版本为 `4`。
...
| `storage` | 必需权限 | 保存和读取全局设置，以及缓存框架检测结果 |
...
| 存在断点的 popup | `Resume index` 操作、已测量页数、恢复提示和进度条 |
| 正在建立索引的 popup | 索引阶段文案、当前测量页数量进度、当前页面标题和进度条 |
| 支持框架的检测流程 | `Detect docs framework` 操作与检测后的框架名称 |
| 文档站侧边栏注入 UI | 顶部总进度卡片、每个页面链接后的百分比徽章、目录占位页 `Subdirectory` 徽章 |
| 单页进度删除交互 | 页面进度徽章 hover 后显示删除图标，并删除该页阅读记录 |
| 页面级记录开关 | 右下角 `Recording` / `Paused` 悬浮开关，以及可拖动位置 |
| 文档页面右侧阅读地图 | 已读区间和当前视口指示 |
| 管理页站点列表 | 已索引站点、页面数、总进度、导出站点操作 |
| 管理页站点详情 | 总进度、页面列表、阅读记录、站点级启用开关 |
| 管理页设置 | 阅读地图开关、新页面默认记录开关、索引调试日志开关、语言选择 |
...
- 目录占位页徽章采用红色提示样式。
- 可删除的页面进度徽章 hover 时采用红色删除提示样式。
- 页面级记录开关为右下角胶囊按钮，启用时使用 teal 状态点，暂停时使用 slate 状态点。
- 右侧阅读地图使用绿色区间标记和视口轮廓。
...
| 框架站点级覆盖配置、Next flight 链接补充和特殊加载等待 | `src/adapters/frameworks/common.ts` 及各框架 adapter |
| 创建索引、后台逐页测量、索引断点和索引过程进度 | `src/background/services/indexing.ts`、`src/storage/indexed-db/index-checkpoints.ts`、`src/indexing/run-progress.ts`、`entrypoints/popup/main.tsx` |
| 授权后后台接续索引 | `src/popup/index-start.ts`、`src/background/services/pending-index.ts` |
| 页面进度卡片、页面徽章、删除单页进度、单页记录开关和右侧阅读地图 | `src/content/progress-ui.ts`、`src/content/reading-tracker.ts`、`entrypoints/content.ts` |
| 单页、站点级和全局阅读记录开关 | `src/settings/page-settings.ts`、`src/settings/site-settings.ts`、`src/settings/app-settings.ts`、`entrypoints/options/app/pages/SettingsPage.tsx` |
| 全局设置与 12 种语言代码 | `src/settings/app-settings.ts`、`src/i18n/messages.ts` |
| 管理页、清理操作、导入导出入口 | `entrypoints/options/app/OptionsApp.tsx`、`entrypoints/options/app/pages/SitesPage.tsx`、`entrypoints/options/app/pages/SettingsPage.tsx`、`entrypoints/options/app/site-detail/*` |
| 便携数据结构与导出文件格式 | `src/storage/portable-data.ts`、`src/background/services/portable-data.ts` |
| IndexedDB 数据库对象仓库和 schema 版本 | `src/storage/indexed-db/connection.ts`、`src/storage/indexed-db/types.ts` |
| 已实现和未实现功能状态 | `TODO.md` |
